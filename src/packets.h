...
// Stores the structure of the header packet
...
struct PacketHeader {
    ...
    uint8_t m_secondaryPlayerCarIndex;        // 255 if no second player 
};

/*
    ********************************************
    ALL THINGS RELATED TO THE MOTION DATA PACKET
    SIZE : 1464 bytes
    ********************************************
*/

// Struct to store the CarMotionData subpacket
...
struct CarMotionData {
    ...
    int16_t m_worldForawrdDirY;                 // World space forward Y direction (normalised)
...
// Struct storing the PacketMotionData 

struct PacketMotionData{
    
    PacketHeader m_header;                      // Packet Header

    CarMotionData m_carMotionData[22];          // Data for all cars on track

    // Extra player can ONLY data
    float m_suspensionPosition[4];              // Note: All wheel arrays have the following order:
    float m_suspensionVelocity[4];              // RL, RR, FL, FR
    float m_suspensionAcceleration[4];          // RL, RR, FL, FR
    float m_wheelSpeed[4];                      // Speed of each wheel
    float m_wheelSlip[4];                       // Slip ratio of each wheel 
...
/*
    *********************************************
    ALL THINGS RELATED TO THE SESSION DATA PACKET
    SIZE : 632 bytes
    *********************************************
*/

// Struct to store the MarshalZones from Session Data
struct MarshalZone {
    float m_zoneStart;                          // Fraction(0...1) of way through the lap the marshal zone starts
    int8_t m_zoneFlag;                          // -1 = invalid/unknown, 0 = none, 1 = green, 2 = blue, 3 = yellow, 4 = red 
};

// Struct to store the Weather Forecase Sample
...
// The structure of the Session Data Packet 
struct PacketSessionData {
    ...
    uint8_t m_sessionType;                      // 0 = unknown, 1 = P1, 2 = P2, 3 = P3, 4 = Short P,
                                                // 5 = Q1, 6 = Q2, 7 = Q3, 8 = Short Q, 9 = OSQ
                                                // 10 = R, 11 = R2, 12 = R3, 13 = Time Trial
    int8_t m_trackId;                           // -1 for unknown, see constants.h
    ...
    uint8_t m_pitSpeedLimit;                    // Pit speed limit in kilometers per hour
    uint8_t m_gamePaused;                       // Whether the game is paused  metwork game only
    ...
    uint8_t m_numWeatherForecastSamples;        // Number of weather forecast samples to follow
    WeatherForecastSample m_weatherForceastSamples[56]; // Array of weather forecase samples
    uint8_t m_forecastAccuracy;                 // 0 = Perfect, 1 = Approximate
    uint8_t m_aiDifficulty;                     // AI Difficulty rating - 0 - 100
    ...
    uint8_t m_gameMode;                         // Game mode id - see constants
    uint8_t m_ruleSet;                          // Ruleset - see constants
...
/*
    *************************************
    ALL THINGS RELAETD TO LAP DATA PACKET
    SIZE : 972 bytes
    *************************************
*/

// Struct to store the Lap Data
struct LapData {
    ...
    uint16_t m_pitLaneTimeInMS;                 // If active, the current time spent in the pit lane in ms
...
// The actual PacketLapData struct

struct PacketLapTimeData {
...
/* 
    *******************************************
    ALL THINGS RELATED TO THE EVENT DATA PACKET
    SIZE : 40 bytes
    *******************************************
*/

// Event details in the packet are different for each type of event!
...
union EventDataDetails {
    ...
    // Penalty
    struct {
        uint8_t penaltyType;                    // Penalty type - see constants
        uint8_t infringementType;               // Infringement type - see constants
        ...
        uint8_t lapNum;                         // Lap the penalty occured on
    ...
    struct {
        ...
        uint8_t isDriverFastestInSession;       // Fastest speed for driver in session = 1, otherise 0
    ...
    struct {
        uint32_t m_buttonStatus;                // Bit flags specifying which buttons are being pressed
                                                // currently - see constants 
    } Buttons;
};

// The actual Event Data Packet 

struct PacketEventData {
    PacketHeader m_header;                      // Header

    uint8_t m_eventStringCode[4];               // Event String Code -  below
    EventDataDetails m_eventDetails;            // Event details - should be interpreted different for each type 
};

// The enum event codes for PacketEventData 
enum class EventCode {
    SSTA,                                       // Seesion Start - Sent when the session starts
    SEND,                                       // Session Ended - Send when the session ends
    ...
    TMPT,                                       // Team mate in pits - Your team mate has entered pits
    CHQF,                                       // Chequered Flag - The chequered flag has been waved
    RCWN,                                       // Race Winner - The race winner is announced
    PENA,                                       // Penalty issed - A penalty has been issued
...
// Helper class to deal with the event codes ENUM 
class EventCodeHelper {
public:

    // Converts the given ENUM expression of event code to a string 
    static constexpr const char* toString(EventCode code) {
        switch(code) {
            case EventCode::SSTA: return "SSTA";
            case EventCode::SEND: return "SEND";
            case EventCode::FTLP: return "FTLP";
            case EventCode::RTMT: return "RTMT";
            case EventCode::DRSE: return "DRSE";
            case EventCode::DRSD: return "DRSD";
            case EventCode::TMPT: return "TMPT";
            case EventCode::CHQF: return "CHQF";
            case EventCode::RCWN: return "RCWN";
            case EventCode::PENA: return "PENA";
            case EventCode::SPTP: return "SPTP";
            case EventCode::STLG: return "STLG";
            case EventCode::LGOT: return "LGOT";
            case EventCode::DTSV: return "DTSV";
            case EventCode::SGSV: return "SGSV";
            case EventCode::FLBK: return "FLBK";
            case EventCode::BUTN: return "BUTN";
            default: return "";
        }
    }

    // Returns the ENUM Event code value from a given string 
    static EventCode fromString(const uint8_t* str) {
        if(std::memcmp(str, "SSTA", 4) == 0) return EventCode::SSTA;
        if(std::memcmp(str, "SEND", 4) == 0) return EventCode::SEND;
        if(std::memcmp(str, "FTLP", 4) == 0) return EventCode::FTLP;
        if(std::memcmp(str, "RTMT", 4) == 0) return EventCode::RTMT;
        if(std::memcmp(str, "DRSE", 4) == 0) return EventCode::DRSE;
        if(std::memcmp(str, "DRSD", 4) == 0) return EventCode::DRSD;
        if(std::memcmp(str, "TMPT", 4) == 0) return EventCode::TMPT;
        if(std::memcmp(str, "CHQF", 4) == 0) return EventCode::CHQF;
        if(std::memcmp(str, "RCWN", 4) == 0) return EventCode::RCWN;
        if(std::memcmp(str, "PENA", 4) == 0) return EventCode::PENA;
        if(std::memcmp(str, "SPTP", 4) == 0) return EventCode::SPTP;
        if(std::memcmp(str, "STLG", 4) == 0) return EventCode::STLG;
        if(std::memcmp(str, "LGOT", 4) == 0) return EventCode::LGOT;
        if(std::memcmp(str, "DTSV", 4) == 0) return EventCode::DTSV;
        if(std::memcmp(str, "SGSV", 4) == 0) return EventCode::SGSV;
        if(std::memcmp(str, "FLBK", 4) == 0) return EventCode::FLBK;
        if(std::memcmp(str, "BUTN", 4) == 0) return EventCode::BUTN;
        throw std::runtime_error("Unknown event code");
    }

    // Sets the event code for an event (unused mostly)
    static void setEventCode(uint8_t* dest, EventCode code) {
        std::memcpy(dest, toString(code), 4);
    }
};

/*
    *********************************************
    ALL THINGS RELATED TO PARTICIPANT DATA PACKET
    SIZE : 1257 bytes
    *********************************************
*/
...
struct ParticipantData {
    ...
    uint8_t m_driverId;                         // Driver id - see constants, 255 if network human
    uint8_t m_networkId;                        // Network id - unique identifier for network players
    uint8_t m_teamId;                           // Team id - see constants
...
// The packet containing participant data 

struct PacketParticipantData {
    
    PacketHeader m_header;                      // Header

    uint8_t m_numActiveCars;                    // Number of active cars in the data - should match number of
                                                // cars on HUD
    ParticipantData m_participants[22];         // All the participants 
};

/*
    ********************************************
    ALL THINGS RELATED TO CAR SETUPS DATA PACKET
    SIZE : 1102 bytes 
    ********************************************

    In Multiplayer, only YOUR own car's setup data will be visible, others will show as blank
*/
...
// The packet containing car setups

struct PacketCarSetupData {
    
    PacketHeader m_header;                      // Header

    CarSetupData m_carSetups[22];               // All the car setups 
};

/*
    ****************************************** 
    ALL THINGS RELATED TO CAR TELEMETRY PACKET
    SIZE : 1347 bytes
    ******************************************
*/
...
struct CarTelemetryData {
    ...
    uint16_t m_speed;                           // Speed of car in kilometers per hour
    float m_throttle;                           // Amount of throttle applied (0.0 to 1.0)
    float m_steer;                              // Steering (-1.0 (full lock left) to 1.0 (full locl right))
    float m_brake;                              // Amount of brake applied (0.0 o 1.0)
    ...
    uint16_t m_revLightsBitValue;               // Rev lights (bit 0 = leftmost LED, but 14 = rightmost LED)
    ...
    uint8_t m_surfaceType[4];                   // Driving surface, see constants 
};
...
struct PacketCarTelemetryData {
    ...
    CarTelemetryData m_carTelemetryData[22];    // All the cars
    
    uint8_t m_mfdPanelIndex;                    // Index of MFP Panel open - 255 = MFD closed
...
/*
    ***************************************
    ALL THINGS RELATED TO CAR STATUS PACKET
    SIZE : 1058 bytes
    ***************************************
*/

// Car Status Data Struct
...
struct CarStatusData {
    ...
    uint8_t m_visualTyreCompound;               // F1 visual (can be different from actual compound)
                                                ...
                                                // F2 - 15 = wet, 19 = super soft, 20 = soft
                                                // 21 = medium, 22 = hard
    ...
    float m_ersDeployedThisLap;                 // ERS energy deployed in this lap
    uint8_t m_networkPaused;                    // Whether the car is paused in a network game 
};

// The packet struct for car status data
struct PacketCarStatusData {
    
    PacketHeader m_header;                      // Header
    
    CarStatusData m_carStatusData[22];          // All the cars
};

/*
    *************************************************
    ALL THINGS RELATED TO FINAL CLASSIFICATION PACKET
    SIZE: 1015 bytes
    *************************************************
*/
...
struct FinalClassificationData {
    ...
    uint8_t m_resultStatus;                     // Results status - 0 = invalid, 1 = inactive, 2 = active
                                                // 3 = finished, 4 = didnotfinish, 5 = disqualified
                                                // 6 = not classfied, 7 = retired
    uint32_t m_bestLapTimeInMS;                 // Best lap time of the sesssion in milliseconds
    double m_totalRaceTime;                     // Total race time in second without penalties
...
struct PacketFinalClassificationData {
    ...
    uint8_t m_numCars;                          // Number of cars in the fnal classification
    FinalClassificationData m_classficationData[22]; 
};

/*
    *******************************************
    ALL THINGS RELATED TO THE LOBBY INFO PACKET
    SIZE : 1191 bytes
    *******************************************
*/
...
struct LobbyInfoData {
    ...
    uint8_t m_aiControlled;                     // Whether the vehicle is AI (1) or Human (0) controlled
    uint8_t m_teamId;                           // Team id - see constants  (255 if no team currently selected)
    uint8_t m_nationality;                      // Nationality of the driver
    char m_name[48];                            // Name of the participant in UTF-8 format - null terminated
                                                // Will be truncated with ... (U+2026) if too long
...
/*
    *******************************************
    ALL THINGS RELATED TO THE CAR DAMAGE PACKET
    SIZE : 948 bytes
    *******************************************
*/
...
struct CarDamageData {
    ...
    uint8_t m_frontRightWngDamage;              // Front right wing damage (percentage)
...
/*
    ********************************************
    ALL THINGS RELATED TO SESSION HISTORY PACKET
    SIZE : 1155 bytes
    ********************************************
*/

// Struct to store the Lap History
...
// Struct storing information about the tyre stints 
struct TyreStintHistoryData {
    
    uint8_t m_endLap;                           // Lap time in milliseconds
...
// The struct that stores the packet data
...
struct PacketSessionHistoryData {
    ...
    uint8_t m_numLaps;                          // Num laps in the data (including partial current lap)
    uint8_t m_numTyreStints;                    // Numer of tyre stints in the data
...
// Union of possible packet types
